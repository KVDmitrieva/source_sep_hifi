...
class CompositeEval:
    def __init__(self, sr=16000):
        self.pesq = PESQMetric()
        self.si_sdr = SISDRMetric()
        self.sdr = SDRMetric()
        self.stoi = STOIMetric()
        self.sr = sr

    def __call__(self, ref_wav, deg_wav):
        ...
        Covl = trim_mos(Covl)
        return {"si-sdr": si_sdr, "pesq": pesq_raw, "sdr": sdr, "stoi": stoi,
                "csig": Csig, "cbak": Cbak, "covl": Covl, "ssnr": segSNR}

    def SSNR(self, ref_wav, deg_wav, srate=16000, eps=1e-10):
    ...
    def wss(self, ref_wav, deg_wav, srate):
        clean_speech = ref_wav
        processed_speech = deg_wav
        clean_length = ref_wav.shape[0]
        processed_length = deg_wav.shape[0]
    ...
    def llr(self, ref_wav, deg_wav, srate):
        clean_speech = ref_wav
        processed_speech = deg_wav
        clean_length = ref_wav.shape[0]
        processed_length = deg_wav.shape[0]
